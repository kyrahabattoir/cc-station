...
/proc/makepowernets()
	...
	if (makingpowernets)
		...
		while (makingpowernets)
			sleep(10)
			// abort abort do not try to be a problem with no ticker!!!
			if (!ticker)
				continue
			if (!ticker.mode)
				continue
			// grace period: 120 seconds
			if (makingpowernetssince + 1200 < ticker.round_elapsed_ticks)
				makingpowernets = 0
				message_admins("Makepowernets still holding the lock after 120 seconds (started at [makingpowernetssince], now is [ticker.round_elapsed_ticks]). Automatically restarting.")
	...
	var/sleep_time = world.timeofday
	...
	for(var/obj/cable/PC in allcables)
		...
		if (abs(world.timeofday - sleep_time) > 2)
			sleep(2)
			sleep_time = world.timeofday
