...
/obj/machinery/bot/firebot/New()
	if (map_setting == "DESTINY" && src.icon == 'icons/obj/aibots.dmi')
		src.icon = 'icons/obj/toolbots.dmi'
	..()
	spawn (5)
		if (src)
	...
	if(!src.target)
		...
		if (!src.target)
			for (var/mob/living/carbon/burningMob in view(7, src))
				...
				if (burningMob.burning || (src.emagged && prob(25)))
...
/obj/machinery/bot/firebot/proc/spray_at(atom/target)
	...
	flick("firebot-c", src)
	if (src.setup_party)
		playsound(src.loc, "sound/items/bikehorn.ogg", 75, 1, -3)
