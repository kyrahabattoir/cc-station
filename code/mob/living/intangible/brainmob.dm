		...
		if(!emote)
			...
			for (var/mob/M in mobs)
				if (istype(M, /mob/new_player))
					continue
				if (M.client && (istype(M, /mob/dead/observer) || (istype(M, /mob/wraith) && !M.density) || (istype(M, /mob/living/intangible/brainmob)) && get_turf(M) in hearers(src)) || ((!isturf(src.loc) && src.loc == M.loc) && !(M in listening) && !istype(M, /mob/dead/target_observer)))
					var/thisR = rendered
