...
/proc/Turfspawn_Wreckage(var/turf/space/center,var/size = 6,var/area/area_restriction = null)
			...
			if (get_dist(S,A) == current_range)
				...
				if (prob(current_chance))
					...
					if (prob(round(current_chance / 2)))
						switch(rand(1,6))
							if(4)
								new /obj/decal/cleanable/robot_debris/gib(locate(S.x, S.y, S.z),0)
							if(5)
								new /obj/decal/cleanable/machine_debris(locate(S.x, S.y, S.z),0)
							if(6)
	...
	switch(RarityClassRoll(100,0,list(90,50)))
		if(1)
			scrap_material = getCachedMaterial(pick("steel","mauxite"))
		if(2)
			scrap_material = getCachedMaterial(pick("cobryl","bohrum"))
		if(3)
			scrap_material = getCachedMaterial(pick("gold","syreline"))
	...
	while(num_items > 0)
		...
		switch(picker)
			if (1 to 3)
				I = new /obj/item/raw_material/scrap_metal(pick(turfs_near_center))
