...
/obj/machinery/computer/genetics/Topic(href, href_list)
	...
	if(href_list["viewpool"])
		...
		info_html = {"<p><b>[GBE.research_level >= 2 ? E.name : "Unknown Mutation"]</b>"}
		...
		info_html = {"<p><b>[GBE.research_level >= 2 ? E.name : "Unknown Mutation"]</b></p><br>"}
		...
		if (E.research_level >= 3 && E.researched_desc)
			info_html = {"<p><b>[E.name]</b><br>[E.researched_desc]</p>"}
		else
			info_html = {"<p><b>[E.name]</b><br>[E.desc]</p>"}
		...
		if(globalInstance != null)
			...
			info_html = "<p><b>[name_string]</b><br>[desc_string]</p>"
		...
		switch(href_list["menu"])
			...
			if("storedmuts")
				...
				for(var/datum/bioEffect/BE in saved_mutations)
					info_html += "<a href='?src=\ref[src];stored_mut=\ref[BE]'><b>Slot [slot]:</b> [BE.name]</a><br>"
					slot++
				info_html += "</p>"
			...
			if("chromosomes")
				...
				for(var/datum/dna_chromosome/C in src.saved_chromosomes)
					info_html += "<a href='?src=\ref[src];stored_chromosome=\ref[C]'><b>[slot]:</b> [C.name]</a><br>"
					slot++
				info_html += "</p>"
			...
			if("combinemuts")
				...
				for(var/datum/bioEffect/BE in saved_mutations)
					info_html += "<a href='?src=\ref[src];mark_for_combination=\ref[BE]'><b>Slot [slot]:</b> [BE.name]</a>"
					if (BE in combining)
						info_html += " *"
					info_html += "<br>"
					slot++
			...
			if (E.can_research)
				if (istext(E.req_mut_research) && GetBioeffectResearchLevelFromGlobalListByID(E.id) < 2)
					...
					if (research_cost > genResearch.researchMaterial)
						info_html += " <i>Material: [research_cost]/[genResearch.researchMaterial]</i></p>"
					else
