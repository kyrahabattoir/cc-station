...
/obj/machinery/chem_heater/attackby(var/obj/item/reagent_containers/glass/B as obj, var/mob/user as mob)
...
/obj/machinery/chem_heater/handle_event(var/event)
	if (event == "reagent_holder_update")
		src.update_icon()
		src.updateUsrDialog()
	return

/obj/machinery/chem_heater/ex_act(severity)
	switch(severity)
		if(1.0)
			qdel(src)
			return
...
/obj/machinery/chem_heater/blob_act(var/power)
	if (prob(25 * power/20))
		qdel(src)

/obj/machinery/chem_heater/meteorhit()
	qdel(src)
	return
...
/obj/machinery/chem_heater/proc/active()
	if(!active)
		return
	if (stat & (NOPOWER|BROKEN))
		src.power_usage = 50
...
/obj/machinery/chem_heater/proc/roboactive(var/obj/item/reagent_containers/glass/B, var/mob/user, var/temperature)
	roboworking = 1
	src.power_usage = 1000
...
/obj/machinery/chem_heater/proc/update_icon()
...
/obj/machinery/chem_dispenser/attackby(var/obj/item/reagent_containers/glass/B as obj, var/mob/user as mob)
...
/obj/machinery/chem_dispenser/handle_event(var/event)
	if (event == "reagent_holder_update")
		src.updateUsrDialog()
	return
...
/obj/machinery/chem_dispenser/blob_act(var/power)
	if (prob(25 * power/20))
		qdel(src)
	return
...
/obj/machinery/chem_dispenser/proc/eject_card()
	if (src.user_id)
		src.user_id.set_loc(get_turf(src))
		src.user_id = null
	return

/obj/machinery/chem_dispenser/proc/update_account()
...
/obj/machinery/chem_dispenser/proc/update_icon()
	if (!beaker)
...
/obj/machinery/chem_master/blob_act(var/power)
	if (prob(25 * power/20))
		qdel(src)
	return
...
/obj/machinery/chem_master/handle_event(var/event)
	if (event == "reagent_holder_update")
		src.updateUsrDialog()
	return

/obj/machinery/chem_master/attackby(var/obj/item/reagent_containers/glass/B as obj, var/mob/user as mob)
	if (!istype(B, /obj/item/reagent_containers/glass))
		return
...
/obj/machinery/chem_master/emag_act(var/mob/user, var/obj/item/card/emag/E)
	if (src.emagged)
		return 0
	if (user)
...
/obj/machinery/chem_master/demag(var/mob/user)
	if (!src.emagged)
		return 0
	if (user)
...
/obj/machinery/chem_master/proc/check_whitelist(var/datum/reagents/R)
...
/obj/machinery/chemicompiler_stationary/New()
	..()
...
/obj/machinery/chemicompiler_stationary/ex_act(severity)
	switch (severity)
		if (1.0)
			qdel(src)
			return
...
/obj/machinery/chemicompiler_stationary/blob_act(var/power)
	if (prob(25 * power/20))
		qdel(src)
	return

/obj/machinery/chemicompiler_stationary/meteorhit()
	qdel(src)
	return

/obj/machinery/chemicompiler_stationary/attack_ai(mob/user as mob)
	return src.attack_hand(user)

/obj/machinery/chemicompiler_stationary/attack_hand(mob/user as mob)
...
/obj/machinery/chemicompiler_stationary/power_change()
	if(stat & BROKEN)
...
/obj/machinery/chemicompiler_stationary/proc/topicPermissionCheck(action)
	if (!(src in range(1)))
		return 0
	if (executor.core.running)
...
/obj/machinery/chemicompiler_stationary/proc/statusChange(oldStatus, newStatus)
	power_change()
	return
