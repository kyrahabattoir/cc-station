...
datum/reagent/disposing()
	holder = null
	..()
	return

datum/reagent/pooled()
	..()
...
datum/reagent/proc/on_add()
	return

datum/reagent/proc/on_remove()
	return

datum/reagent/proc/grenade_effects(var/obj/grenade, var/atom/A)
	return

datum/reagent/proc/reaction_temperature(exposed_temperature, exposed_volume) //By default we do nothing.
	return

datum/reagent/proc/reaction_blob(var/obj/blob/B, var/volume)
	if (!blob_damage)
		return
	B.take_damage(blob_damage, volume, "poison")
	return

datum/reagent/proc/reaction_mob(var/mob/M, var/method=REAC_TOUCH, var/volume)	//By default we have a chance to transfer some
...
datum/reagent/proc/on_mob_life(var/mob/M)
	if (!M || !M.reagents)
		return
	if (!holder)
		holder = M.reagents
...
datum/reagent/proc/on_plant_life(var/obj/machinery/plantpot/P)
	if(!P)
		return
	return //why is this here?

datum/reagent/proc/check_overdose(var/mob/M)
	if (!M || !M.reagents)
		return
	if (!holder)
		holder = M.reagents
...
datum/reagent/proc/do_overdose(var/severity, var/mob/M)
	...
	if (ismob(severity))
		return				//Wire: Fix for shitty fucking byond mixing up vars
	var/effect = rand(1, 100) - severity
	if (effect <= 8)
		M.take_toxin_damage(severity)
	return effect

datum/reagent/proc/on_transfer(var/datum/reagents/source, var/datum/reagents/target, var/trans_amt)
...
datum/reagent/proc/is_solid()
	return reagent_state == REAGENT_SOLID

datum/reagent/proc/is_liquid()
	return reagent_state == REAGENT_LIQUID

datum/reagent/proc/is_gas()
	return reagent_state == REAGENT_GAS

datum/reagent/proc/physical_shock(var/force)
	return
