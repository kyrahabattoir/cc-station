...
datum/reagent/nitroglycerin // Yes, this is a bad idea.
	name = "nitroglycerin"
	id = "nitroglycerin"
...
datum/reagent/nitroglycerin/proc/explode(var/turf/T, expl_reason, del_holder=1)
...
datum/reagent/nitroglycerin/reaction_turf(var/turf/T, var/volume)
	explode(T, "splash on turf")

datum/reagent/nitroglycerin/reaction_mob(var/mob/M, var/volume)
	explode(get_turf(M), "splash on [key_name(M)]")

datum/reagent/nitroglycerin/reaction_obj(var/obj/O, var/volume)
...
datum/reagent/aranesp/on_remove()
...
datum/reagent/aranesp/on_mob_life(var/mob/M)
	if (!M)
		M = holder.my_atom
...
datum/reagent/hairgrownium //It..grows hair.  Not to be confused with the previous hair growth reagent, "wacky monkey cheeseonium"
	name = "hairgrownium"
	id = "hairgrownium"
...
datum/reagent/hairgrownium/on_mob_life(var/mob/M)
	if (!M)
		M = holder.my_atom
...
datum/reagent/super_hairgrownium //moustache madness
...
datum/reagent/super_hairgrownium/on_mob_life(var/mob/M)
...
datum/reagent/anima //This stuff is not done. Don't use it. Don't spoil it.
	name = "anima"
	id = "anima"
...
datum/reagent/anima/on_mob_life(var/mob/M)
	if (!M)
		M = holder.my_atom
...
datum/reagent/strange_reagent/reaction_obj(var/obj/O, var/volume)
	src = null
...
datum/reagent/strange_reagent/on_mob_life(var/mob/M)
	if (!M)
		M = holder.my_atom
...
datum/reagent/carpet/reaction_turf(var/turf/T, var/volume)
	if (T.icon == 'icons/turf/floors.dmi' && volume >= 5)
		spawn(15)
			T.icon_state = "grimy"
	src = null

datum/reagent/fffoam
...
datum/reagent/fffoam/reaction_temperature(exposed_temperature, exposed_volume)
	return

datum/reagent/fffoam/reaction_turf(var/turf/target, var/volume)
	src = null
...
datum/reagent/fffoam/reaction_obj(var/obj/item/O, var/volume)
	src = null
	if (istype(O) && prob(40))
		if (O.burning)
			O.burning = 0
...
datum/reagent/lube/reaction_turf(var/turf/target, var/volume)
	src = null
	var/turf/simulated/T = target
	if (istype(T))
		if (T.wet >= 2) return
...
datum/reagent/ammonia/on_plant_life(var/obj/machinery/plantpot/P)
	P.growth += 4
	if (prob(66))
		P.health++
...
datum/reagent/diethylamine/on_plant_life(var/obj/machinery/plantpot/P)
	if (prob(66))
		P.growth+=2
...
datum/reagent/acetone/on_mob_life(var/mob/M)
	if (!M)
		M = holder.my_atom
...
datum/reagent/ectoplasm/on_mob_life(var/mob/M)
	if (!M)
		M = holder.my_atom
...
datum/reagent/cryostylane/on_mob_life(var/mob/M)
	if (!M)
		M = holder.my_atom
...
datum/reagent/cryostylane/reaction_turf(var/turf/target, var/volume)
	src = null
	if (volume >= 3)
...
datum/reagent/booster_enzyme/on_mob_life(var/mob/M)
...
datum/reagent/space_cleaner // COGWERKS CHEM REVISION PROJECT. ethanol, ammonia + water - treat like a shitty version of windex
	name = "space cleaner"
	id = "cleaner"
...
datum/reagent/oil/reaction_turf(var/turf/target, var/volume)
	var/turf/simulated/T = target
...
datum/reagent/oil/on_mob_life(var/mob/M)
...
datum/reagent/capulettium/pooled()
	..()
	counter = 1

datum/reagent/capulettium/on_mob_life(var/mob/M)
	if (!M)
		M = holder.my_atom
...
datum/reagent/capulettium_plus/pooled()
	..()
	counter = 1

datum/reagent/capulettium_plus/on_mob_life(var/mob/M)
	if (!M)
		M = holder.my_atom
...
datum/reagent/denatured_enzyme
...
datum/reagent/werewolf_serum_fake3
...
datum/reagent/werewolf_serum_fake4
...
datum/reagent/werewolf_serum_fake4/on_mob_life(var/mob/M)
	if (!M)
		M = holder.my_atom
...
datum/reagent/sewage/on_mob_life(var/mob/M)
	if (!M)
		M = holder.my_atom
...
datum/reagent/ants/on_mob_life(var/mob/M)
	if (!M)
		M = holder.my_atom
...
datum/reagent/spiders/reaction_turf(var/turf/T, var/volume)
...
datum/reagent/spiders/on_mob_life(var/mob/M)
	if (!M)
		M = holder.my_atom
...
datum/reagent/love/reaction_mob(var/mob/M)
	boutput(M, "<span style=\"color:blue\">You feel loved!</span>")

datum/reagent/love/on_mob_life(var/mob/M)
	if (!M)
		M = holder.my_atom
...
datum/reagent/colors/reaction_obj(var/obj/O, var/volume)
	src = null
	O.color = rgb(rand(0,255),rand(0,255),rand(0,255))

datum/reagent/colors/reaction_turf(var/turf/T, var/volume)
	src = null
	T.color = rgb(rand(0,255),rand(0,255),rand(0,255))

datum/reagent/shark_dna
...
datum/reagent/paper/reaction_turf(var/turf/T, var/volume)
	src = null
...
datum/reagent/fliptonium/pooled()
	..()
	remove_buff = 0
	direction = null
	dir_lock = 0
...
datum/reagent/fliptonium/on_mob_life(var/mob/M)
	if (!M)
		M = holder.my_atom
...
datum/reagent/fliptonium/reaction_mob(var/mob/M)
...
/*
datum/reagent/fliptonium/reaction_obj(var/obj/O, var/volume)
	if (volume >= 10)
		var/dir_temp = pick("L", "R")
		var/speed_temp = text2num("[rand(1,6)].[rand(0,9)]")
...
datum/reagent/fliptonium/on_add()
	if (istype(holder) && istype(holder.my_atom) && hascall(holder.my_atom,"add_stam_mod_regen"))
		remove_buff = holder.my_atom:add_stam_mod_regen("consumable_good", 2)

datum/reagent/fliptonium/on_remove()
	if (remove_buff)
		if (istype(holder) && istype(holder.my_atom) && hascall(holder.my_atom,"remove_stam_mod_regen"))
...
datum/reagent/fliptonium/do_overdose(var/severity, var/mob/M)
...
datum/reagent/fliptonium/glowing_fliptonium
...
datum/reagent/fliptonium/glowing_fliptonium/on_mob_life(var/mob/M)
	if (!M)
		M = holder.my_atom
...
datum/reagent/fliptonium/glowing_fliptonium/reaction_obj(var/obj/O)
	var/dir_temp = pick("L", "R")
	var/speed_temp = text2num("[rand(0,10)].[rand(0,9)]")
	animate_spin(O, dir_temp, speed_temp)

datum/reagent/fliptonium/glowing_fliptonium/reaction_turf(var/turf/T) // oh god what am I doing this is such a bad idea
	var/dir_temp = pick("L", "R")
	var/speed_temp = text2num("[rand(0,10)].[rand(0,9)]")
	animate_spin(T, dir_temp, speed_temp)

datum/reagent/fliptonium/glowing_fliptonium/on_add()
	if (istype(holder) && istype(holder.my_atom) && hascall(holder.my_atom,"add_stam_mod_regen"))
		remove_buff = holder.my_atom:add_stam_mod_regen("consumable_good", 4)

datum/reagent/fliptonium/glowing_fliptonium/on_remove()
	if (remove_buff)
		if (istype(holder) && istype(holder.my_atom) && hascall(holder.my_atom,"remove_stam_mod_regen"))
			holder.my_atom:remove_stam_mod_regen("consumable_good")
	if (istype(holder) && istype(holder.my_atom))
...
datum/reagent/diluted_fliptonium
...
datum/reagent/diluted_fliptonium/on_mob_life(var/mob/M)
	if (!M)
		M = holder.my_atom
...
datum/reagent/fartonium // :effort:
	name = "fartonium"
	id = "fartonium"
...
datum/reagent/fartonium/on_mob_life(var/mob/M)
	if (!M)
		M = holder.my_atom
...
datum/reagent/flaptonium/reaction_obj(var/obj/O, var/volume)
	var/turf/T = get_turf(O)
	createSomeBirds(T, volume)

datum/reagent/flaptonium/reaction_turf(var/turf/T, var/volume)
	createSomeBirds(T, volume)

datum/reagent/flaptonium/proc/createSomeBirds(var/turf/T as turf, var/volume)
	fucking_critter_bullshit_fuckcrap_limiter(reaction_count)
	if (!T)
		return
	if (volume < 5)
...
datum/reagent/glitter/reaction_turf(var/turf/T, var/volume)
	src = null
	if (!istype(T, /turf/space))
		if (volume >= 5)
...
datum/reagent/glitter/on_remove()
	var/atom/A = holder.my_atom
	if (ismob(A))
		if (particleMaster.CheckSystemExists(/datum/particleSystem/glitter, A))
...
datum/reagent/glitter/on_mob_life(var/mob/M)
	if (!M)
		M = holder.my_atom
...
datum/reagent/glitter_harmless // no recipe, doesn't do any damage
	name = "glitter"
	id = "glitter_harmless"
	description = "Fabulous!"
	reagent_state = SOLID
...
datum/reagent/glitter_harmless/on_add()
	var/atom/A = holder.my_atom
	if (ismob(A))
		if (!particleMaster.CheckSystemExists(/datum/particleSystem/glitter, A))
			particleMaster.SpawnSystem(new /datum/particleSystem/glitter(A))
...
datum/reagent/glitter_harmless/reaction_turf(var/turf/T, var/volume)
	src = null
	if (!istype(T, /turf/space))
		if (volume >= 5)
			if (!locate(/obj/decal/cleanable/glitter) in T)
...
datum/reagent/glitter_harmless/on_remove()
	var/atom/A = holder.my_atom
	if (ismob(A))
		if (particleMaster.CheckSystemExists(/datum/particleSystem/glitter, A))
			particleMaster.RemoveSystem(/datum/particleSystem/glitter, A)
...
datum/reagent/green_goop // lets you see ghosts while it's in you.  exists only for ectocooler to decay into atm
...
datum/reagent/voltagen/pooled()
	..()
	multiplier = 1
	grenade_handled = 0

datum/reagent/voltagen/grenade_effects(var/obj/grenade, var/atom/A)
...
datum/reagent/voltagen/on_mob_life(mob/M)
...
datum/reagent/voltagen/do_overdose(var/severity, var/mob/M)
	if (prob(10))
		if (severity >= 2)
...
datum/reagent/weedkiller/on_mob_life(var/mob/M) // cogwerks note. making atrazine toxic
	if (!M)
		M = holder.my_atom
...
datum/reagent/weedkiller/on_plant_life(var/obj/machinery/plantpot/P)
...
datum/reagent/ash/on_plant_life(var/obj/machinery/plantpot/P)
	if (prob(80))
		P.growth++
	if (prob(80))
		P.health++
...
datum/reagent/potash/on_plant_life(var/obj/machinery/plantpot/P)
	/*if (prob(80))
		P.growth+=2
	if (prob(80))
		P.health+=2
...
datum/reagent/plant_nutrients/on_plant_life(var/obj/machinery/plantpot/P)
	if (prob(80))
		P.growth+=3
	if (prob(80))
		P.health+=3
...
datum/reagent/blood/reaction_turf(var/turf/T, var/volume)
	src = null
	if (volume >= 5)
...
datum/reagent/blood/on_mob_life(var/mob/M)
...
datum/reagent/blood/on_plant_life(var/obj/machinery/plantpot/P)
	var/datum/plant/growing = P.current
	if (growing.growthmode == "carnivore")
		P.growth += 3

datum/reagent/blood/on_transfer(var/datum/reagents/source, var/datum/reagents/target, var/trans_amt)
...
datum/reagent/blood/bloodc
	id = "bloodc"
	value = 3
	hygiene_value = -4

datum/reagent/blood/bloodc/reaction_temperature(exposed_temperature, exposed_volume)
...
datum/reagent/gvomit/reaction_turf(var/turf/T, var/volume)
	src = null
	if (volume >= 5)
...
/*
datum/reagent/urine/on_mob_life(var/mob/M) //why
...
datum/reagent/triplepiss/reaction_turf(var/turf/T, var/volume)
	src = null
	if (volume >= 5)
		if (!locate(/obj/decal/cleanable/urine) in T)
			playsound(T, "sound/effects/splat.ogg", 50, 1)
...
datum/reagent/poo/on_plant_life(var/obj/machinery/plantpot/P)
	if (prob(66))
		P.health++

datum/reagent/big_bang_precursor
...
datum/reagent/big_bang/reaction_turf(var/turf/T, var_volume)
	src = null
	T.ex_act(1)
...
datum/reagent/big_bang/reaction_obj(var/obj/O, var/volume)
	//if (!istype(O, /obj/effects/foam)
	//	&& !istype(O, /obj/item/reagent_containers)
	//	&& !istype(O, /obj/item/chem_grenade))
	O.ex_act(1)

datum/reagent/big_bang/on_mob_life(var/mob/M)
	..(M)
	M.ex_act(1)

datum/reagent/cyclopentanol
	name = "cyclopentanol"
	id = "cyclopentanol"
...
datum/reagent/magnesium_chloride
...
datum/reagent/reversium/on_mob_life(var/mob/M)
	if(!M) M = holder.my_atom
	M.bioHolder.AddEffect("reversed_speech", timeleft = 180)
	..(M)
