...
datum/reagent/drug/bathsalts/pooled()
	..()
	remove_buff = 0

datum/reagent/drug/bathsalts/on_add()
...
datum/reagent/drug/bathsalts/on_remove()
	if(remove_buff)
...
datum/reagent/drug/bathsalts/on_mob_life(var/mob/M) // commence bad times
	if(!M)
		M = holder.my_atom
...
datum/reagent/drug/bathsalts/reaction_mob(var/mob/M, var/method=REAC_TOUCH, var/volume)
...
datum/reagent/drug/bathsalts/do_overdose(var/severity, var/mob/M)
...
datum/reagent/drug/jenkem/on_mob_life(var/mob/M)
	if(!M)
		M = holder.my_atom
	if(ishuman(M))
		var/mob/living/carbon/human/H = M
...
datum/reagent/drug/crank/on_mob_life(var/mob/M)
...
datum/reagent/drug/crank/do_overdose(var/severity, var/mob/M)
	var/effect = ..(severity, M)
	if (severity == 1)
		if (effect <= 2)
...
datum/reagent/drug/LSD/on_mob_life(var/mob/M)
	if(!M)
		M = holder.my_atom
...
datum/reagent/drug/LSD/reaction_mob(var/mob/M, var/method=REAC_TOUCH, var/volume)
	if(method != REAC_INGEST)
		return
...
datum/reagent/drug/space_drugs/reaction_temperature(exposed_temperature, exposed_volume)
...
datum/reagent/drug/space_drugs/on_mob_life(var/mob/M)
	if(!M) 
		M = holder.my_atom
	if(ishuman(M))
		var/mob/living/carbon/human/H = M
...
datum/reagent/drug/THC/on_mob_life(var/mob/M)
	if(!M)
		M = holder.my_atom
	if(ishuman(M))
		var/mob/living/carbon/human/H = M
...
datum/reagent/drug/nicotine/pooled()
	..()
	remove_buff = 0

datum/reagent/drug/nicotine/on_add()
	if(!istype(holder))
		return
...
datum/reagent/drug/nicotine/on_remove()
	if(!remove_buff)
		return
	if(!istype(holder))
		return
...
datum/reagent/drug/nicotine/on_mob_life(var/mob/M)
	if(ishuman(M))
		var/mob/living/carbon/human/H = M
		if (H.sims)
...
datum/reagent/drug/nicotine/do_overdose(var/severity, var/mob/M)
	var/effect = ..(severity, M)
	if (severity == 1)
		if (effect <= 2)
...
datum/reagent/drug/psilocybin/on_mob_life(var/mob/M)
	if(!M)
		M = holder.my_atom
	if(ishuman(M))
		var/mob/living/carbon/human/H = M
...
datum/reagent/drug/krokodil/on_mob_life(var/mob/M)
	if(!M)
		M = holder.my_atom
	if(ishuman(M))
		var/mob/living/carbon/human/H = M
...
datum/reagent/drug/krokodil/do_overdose(var/severity, var/mob/M)
	var/effect = ..(severity, M)
	if (severity == 1)
		if (effect <= 2)
...
datum/reagent/drug/catdrugs/on_mob_life(var/mob/M)
	if(!M)
		M = holder.my_atom
	if(ishuman(M))
		var/mob/living/carbon/human/H = M
...
datum/reagent/drug/catdrugs/reaction_mob(var/mob/M, var/method=REAC_TOUCH, var/volume)
	if(method != REAC_INGEST)
		return
	M.playsound_local(M.loc, pick('sound/effects/cat.ogg', 'sound/effects/cat_hiss.ogg'), 50, 1)
	boutput(M, "<span style=\"color:red\"><font face='[pick("Arial", "Georgia", "Impact", "Mucida Console", "Symbol", "Tahoma", "Times New Roman", "Verdana")]' size='[rand(3,6)]'>Holy shit, you start tripping balls!</font></span>")
...
datum/reagent/drug/triplemeth/on_add()
	return

datum/reagent/drug/triplemeth/on_remove()
	if(istype(holder) && istype(holder.my_atom) && hascall(holder.my_atom,"remove_stam_mod_regen"))
...
datum/reagent/drug/triplemeth/on_mob_life(var/mob/M)
	if(!M)
		M = holder.my_atom
	if(ishuman(M))
		var/mob/living/carbon/human/H = M
	...
	if(holder.has_reagent("methamphetamine"))
		return ..(M) //Since is created by a meth overdose, dont react while meth is in their system.
...
datum/reagent/drug/triplemeth/do_overdose(var/severity, var/mob/M)
	var/effect = ..(severity, M)
	if(holder.has_reagent("methamphetamine"))
		return ..(M) //Since is created by a meth overdose, dont react while meth is in their system.
...
datum/reagent/drug/methamphetamine // COGWERKS CHEM REVISION PROJECT. marked for revision
...
datum/reagent/drug/methamphetamine/pooled()
	..()
	remove_buff = 0
	return

datum/reagent/drug/methamphetamine/on_add()
	if(istype(holder) && istype(holder.my_atom) && hascall(holder.my_atom,"add_stam_mod_regen"))
		remove_buff = holder.my_atom:add_stam_mod_regen("consumable_good", 3)
	return

datum/reagent/drug/methamphetamine/on_remove()
	if(remove_buff)
		if(istype(holder) && istype(holder.my_atom) && hascall(holder.my_atom,"remove_stam_mod_regen"))
			holder.my_atom:remove_stam_mod_regen("consumable_good")
...
datum/reagent/drug/methamphetamine/on_mob_life(var/mob/M)
	if(!M) 
		M = holder.my_atom
	if(ishuman(M))
		var/mob/living/carbon/human/H = M
...
datum/reagent/drug/methamphetamine/do_overdose(var/severity, var/mob/M)
	var/effect = ..(severity, M)
	if (severity == 1)
		if (effect <= 2)
			M.visible_message("<span style=\"color:red\"><b>[M.name]</b> can't seem to control their legs!</span>")
