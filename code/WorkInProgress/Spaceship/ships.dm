...
/obj/machinery/vehicle/pod_smooth
	...
	onMaterialChanged()
		...
		if(istype(src.material))
			src.maxhealth = max(75, src.material.getProperty(PROP_TOUGHNESS) * 5)
			src.health = maxhealth
			src.speed = max(0.75,min(5, (100 - src.material.getProperty(PROP_DIELECTRIC)) / 40))
		return
...
/obj/machinery/vehicle/escape_pod
	...
	relaymove(mob/user as mob, direction) //haha nope
		return
