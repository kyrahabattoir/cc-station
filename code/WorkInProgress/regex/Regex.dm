...
regex
	...
	New(regex/p,s=1,regex/first,regex/last)
		...
		while(index<end && !first.error)
			...
			continue
			almostdone:
			if(index >= end) goto done
			ch=text2ascii(pattern,index)
			if(ch == endchar) continue	// let endchar handling pick this up
			goto done
			...
			switch(ptype)
				if(1,13,14)	// string
					...
					stringmatch:
						...
						while(terrible_serverdeath_inhibitor-- && (times<m || m<0))
