...
/client/proc/cmd_admin_playermode()
	...
	if (player_mode)
		...
		if (choice == "NONE" || !choice)
			...
			player_mode_ahelp = 0
			player_mode_mhelp = 0
			boutput(usr, "<span style=\"color:blue\">Player mode now ON.</span>")
		switch (choice)
			if ("ASAY, AHELP & MHELP")
				player_mode = 1
				player_mode_asay = 1
				player_mode_ahelp = 1
				player_mode_mhelp = 1
				boutput(usr, "<span style=\"color:blue\">Player mode now ON. You will recieve: ASAYs, AHELPs, MHELPs.</span>")
			if ("ASAY & AHELP")
				player_mode = 1
				player_mode_asay = 1
				player_mode_ahelp = 1
				player_mode_mhelp = 0
				boutput(usr, "<span style=\"color:blue\">Player mode now ON. You will recieve: ASAYs, AHELPs.</span>")
			if ("ASAY & MHELP")
				player_mode = 1
				player_mode_asay = 1
				player_mode_ahelp = 0
				player_mode_mhelp = 1
				boutput(usr, "<span style=\"color:blue\">Player mode now ON. You will recieve: ASAYs, MHELPs.</span>")
			if ("AHELP & MHELP")
				player_mode = 1
				player_mode_asay = 0
				player_mode_ahelp = 1
				player_mode_mhelp = 1
				boutput(usr, "<span style=\"color:blue\">Player mode now ON. You will recieve: AHELPs, MHELPs.</span>")
			if ("ASAY ONLY")
				player_mode = 1
				player_mode_asay = 1
				player_mode_ahelp = 0
				player_mode_mhelp = 0
				boutput(usr, "<span style=\"color:blue\">Player mode now ON. You will recieve: ASAYs.</span>")
			if ("AHELP ONLY")
				player_mode = 1
				player_mode_asay = 0
				player_mode_ahelp = 1
				player_mode_mhelp = 0
				boutput(usr, "<span style=\"color:blue\">Player mode now ON. You will recieve: AHELPs.</span>")
			if ("MHELP ONLY")
				player_mode = 1
				player_mode_asay = 0
				player_mode_ahelp = 0
				player_mode_mhelp = 1
				boutput(usr, "<span style=\"color:blue\">Player mode now ON. You will recieve: MHELPs.</span>")
...
/client/proc/cmd_admin_djmode()
	...
	if (djmode)
		djmode = 0
		boutput(usr, "<span style=\"color:blue\">DJ mode now OFF.</span>")
	else
		djmode = 1
		boutput(usr, "<span style=\"color:blue\">DJ mode now ON.</span>")
