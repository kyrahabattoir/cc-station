(function (window, document, $) {
	...
	*/
	...
	function parseDna(jqTarget, dnaData, bRaw) {
		if(bRaw === undefined) {bRaw = false;}
		var t = $(jqTarget);
		t.empty();
	...
	function shiftSelectedSpliceSeq(jqT, dir) {
		...
		sendSelectionUpdate(curSel.index(), t.attr("id") === "spliceTargetField");
		updateSpliceControls();
	}
	...
	function doSpliceAction(dir) {
		...
		var s = $("#txtSpliceSource .splice-selected"),
			...
			iTargetPos =0; //The index holding the value we add
			...
			updateSpliceControls();
			xmit({splicemod:1, SourcePos:iSourcePos,pos:iPos,rel:iRel}, "setUIState");
	}
	...
	function selectSpliceByIndex(index, container) {
		index--; //BYOND's index is 1-based
		var c = $(".splice-ent", $(container));
		if(index >= 0 && index < c.length) {
			c.removeClass("splice-selected");
			$(c.get(index)).addClass("splice-selected");
		}
		
	}
	
	function sendSelectionUpdate(index, isTarget) {
		var d = {splicesel:index};
	...
	function handleSpliceSequenceClick(clicked) {
		...
		sendSelectionUpdate($(clicked).index(), tf.attr("id") === "txtSpliceTarget");
		updateSpliceControls();
	}
	...
	function setUIState(data) {
		...
		if(data.hasOwnProperty("splice")) {
			...
			if(s.hasOwnProperty("source") || s.hasOwnProperty("target")) {
				...
				updateCompleteSpliceData(target, false, source, false);
				updateSpliceControls();
			}
			...
			if(s.hasOwnProperty("selSource")) {
				selectSpliceByIndex(s.selSource, "#txtSpliceSource");
			}
			if(s.hasOwnProperty("selTarget")) {
				selectSpliceByIndex(s.selTarget, "#txtSpliceTarget");
			}
		}
